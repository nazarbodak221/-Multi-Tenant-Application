...
class TestFullFlow:
    ...
    async def test_complete_flow(self, core_db, test_client: AsyncClient):
        ...
        profile_response = await test_client.get(
            "/api/v1/users/me",
            headers={"Authorization": f"Bearer {tenant_token}", "X-Tenant-Id": org_id},
        )
        ...
        update_response = await test_client.put(
            "/api/v1/users/me",
            headers={"Authorization": f"Bearer {tenant_token}", "X-Tenant-Id": org_id},
            json={"full_name": "Updated Employee", "phone": "+1234567890"},
        )
